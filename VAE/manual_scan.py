...
if __name__ == '__main__':
    ...
    best_model = 0
    best_ind = (-1, -1, -1, -1, -1)
    val_losses = np.zeros((len(ker1_vals),len(ker2_vals), len(ker3_vals), len(conv1_vals), len(conv2_vals)))
    best_hist = np.array([0])
    pred_times = np.zeros((len(ker1_vals),len(ker2_vals), len(ker3_vals), len(conv1_vals), len(conv2_vals)))
    ...
    count = 0
    for ker1_in, ker1_val in enumerate(ker1_vals):
        for ker2_in, ker2_val in enumerate(ker2_vals):
            for ker3_in, ker3_val in enumerate(ker3_vals):
                for conv1_in, conv1_val in enumerate(conv1_vals):
                    for conv2_in, conv2_val in enumerate(conv2_vals):
                        ...
                        x = layers.Conv2D(conv1_val, ker1_val, activation="relu", padding="same")(input)
                        x = layers.MaxPooling2D((2, 2), padding="same")(x)
                        x = layers.Conv2D(conv2_val, ker2_val, activation="relu", padding="same")(x)
                        x = layers.MaxPooling2D((2, 2), padding="same")(x)

                        x = layers.Conv2DTranspose(conv2_val, ker2_val, strides=2, activation="relu", padding="same")(x)
                        x = layers.Conv2DTranspose(conv1_val, ker1_val, strides=2, activation="relu", padding="same")(x)
                        x = layers.Conv2D(1, ker3_val, activation="sigmoid", padding="same")(x)
                        ...
                        val_losses[ker1_in, ker2_in, ker3_in, conv1_in, conv2_in] = val_loss
                        
                        # See if best validation loss
                        if val_loss < best_val_loss:
                            best_val_loss = val_loss
                            best_model = autoencoder
                            best_ind = (ker1_in, ker2_in, ker3_in, conv1_in, conv2_in)
                            best_hist = hist
                        ...
                        t_predict /= chn_num
                        pred_times[ker1_in, ker2_in, ker3_in, conv1_in, conv2_in] = t_predict / chn_num
    ...
    data_path = f'/scratch/gpfs/ar0535/spec_model_data/sweep/'
    ...
    ker1_avg_loss = np.zeros((len(ker1_vals),1))
    ker2_avg_loss = np.zeros((len(ker2_vals),1))
    ker3_avg_loss = np.zeros((len(ker3_vals),1))
    conv1_avg_loss = np.zeros((len(conv1_vals),1))
    conv2_avg_loss = np.zeros((len(conv2_vals),1))
    ker1_pred_avg = np.zeros((len(ker1_vals),1))
    ker2_pred_avg = np.zeros((len(ker2_vals),1))
    ker3_pred_avg = np.zeros((len(ker3_vals),1))
    conv1_pred_avg = np.zeros((len(conv1_vals),1))
    conv2_pred_avg = np.zeros((len(conv2_vals),1))
    
    # Kernel 1
    for ker1_in in range(len(ker1_vals)):
        # Val loss
        flat = val_losses[ker1_in,:,:,:,:].flatten()
        ker1_avg_loss[ker1_in] = np.average(flat)
        
        # Pred time
        flat = pred_times[ker1_in,:,:,:,:].flatten()
        ker1_pred_avg[ker1_in] = np.average(flat)
        
    # Kernel 2
    for ker2_in in range(len(ker2_vals)):
        # Val Loss
        flat = val_losses[:,ker2_in,:,:,:].flatten()
        ker2_avg_loss[ker2_in] = np.average(flat)
        
        # Pred time
        flat = pred_times[:,ker2_in,:,:,:].flatten()
        ker2_pred_avg[ker2_in] = np.average(flat)
        
    # Kernel 3 
    for ker3_in in range(len(ker3_vals)):
        flat = val_losses[:,:,ker3_in,:,:].flatten()
        ker3_avg_loss[ker3_in] = np.average(flat)
        
        # Pred time
        flat = pred_times[:,:,ker3_in,:,:].flatten()
        ker3_pred_avg[ker3_in] = np.average(flat)
    ...
    np.savez(data_path+'loss_comparisons.npz', ker1_loss=ker1_avg_loss, ker2_loss=ker2_avg_loss, 
             ker3_loss=ker3_avg_loss, conv1_loss=conv1_avg_loss, conv2_loss=conv2_avg_loss, 
            ker1_time=ker1_pred_avg, ker2_time=ker2_pred_avg, ker3_time=ker3_pred_avg, 
            conv1_time=conv1_pred_avg, conv2_time=conv2_pred_avg)
