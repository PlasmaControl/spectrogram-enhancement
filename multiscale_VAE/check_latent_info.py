...
def patch(arr, window_size, num_strips):
    ...
    all_patches = np.empty((num_specs * num_strips, width, height))
...
def process_inputs(x, y, window_size, num_strips):    
    ...
    x_test = np.transpose(x_test, axes=[0,1,3,2])
    x_train = np.transpose(x_train, axes=[0,1,3,2])
    x_valid = np.transpose(x_valid, axes=[0,1,3,2])
